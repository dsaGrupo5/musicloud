...
public interface CancionDAOQuery {
    ...
    public final static String OBTENER_COLECCION_CANCIONES_APARTIR_ID_PAGINADA_A_5 =       "select hex(id) as id,artista, nombre, genero,url, last_modified, creation_timestamp from canciones where creation_timestamp < ?  order by ? asc limit 20";
    public final static String OBTENER_COLECCION_CANCIONES_APARTIR_ID_PAGINADA_A_5_after = "select hex(id) as id,artista, nombre, genero,url, last_modified, creation_timestamp from canciones where creation_timestamp > ?  order by ? asc limit 20";
